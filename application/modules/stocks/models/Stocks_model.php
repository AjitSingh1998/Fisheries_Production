...
class Stocks_model extends CI_Model {
	...
	function get_fisherman($q, $f_ids = '', $offset=0, $limit=10){
		...
		$query = "SELECT
				  ...
				  WHERE fisherman.status = 'Active' AND (fisherman.NAME like '%".$q."%' OR fisherman.Code like '%".$q."%') ".(($f_ids != '')?' AND fisherman.ID NOT IN('.$f_ids.')':'').
	...
	function get_mgtype_data(){
		$data = array();
		$result = $this->db->get_where(MAINGROUP_TYPE, array('status'=>'Active'))->result_array();
		if(!empty($result)){
			$data = $result;
		}
		//printr($data);
	...
	function get_mg_data($type){
		$data = array();
		$result = $this->db->get_where(MAINGROUP, array('Type'=>$type, 'status'=>'Active'))->result_array();
		if(!empty($result)){
			$data = $result;
		}
		//printr($data);
	...
	function get_all_mg_fisherman($maingroup, $f_ids = ''){
		...
		$query = "SELECT
				  ...
				  FROM `".FISHERMAN."` as fisherman
				  LEFT JOIN `".MAINGROUP."` as maingroup ON maingroup.ID = fisherman.MainGroup
				  WHERE fisherman.status = 'Active' AND (fisherman.MainGroup = ".$maingroup." ) ".(($f_ids != '')?' AND fisherman.ID NOT IN('.$f_ids.')':'').
				  " ORDER BY fisherman.Code ASC";
		$result = $this->db->query($query)->result_array();
		if(!empty($result)){
			$new_data = array();
	...
	function get_pr_items($cat_id){
		$data = array();
		$result = $this->db->get_where(PRODUCT, array('Category_Id'=>$cat_id, 'status'=>'Active'))->result_array();
		if(!empty($result)){
			$data = $result;
		}
		//printr($data);
	...
	function get_pr_cat($ptype){
		$data = array();
		$result = $this->db->get_where(PRODUCT_CATEGORY, array('Type_id'=>$ptype, 'status'=>'Active'))->result_array();
		if(!empty($result)){
			$data = $result;
		}
		//printr($data);
