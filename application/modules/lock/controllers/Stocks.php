...
class Stocks extends MY_Controller {
	...
	//Listing
	function outward(){	
		$this->actions = checkUserPermission('lock/stocks/outward', $this->uri->segment(4));
	...
	function outward_return($id = NULL){
		...
		$data['scriptsrc'] = array(site_url('lock/assets/js/outward.js'));
	...
	//Listing
	function inward(){
		$actions = checkUserPermission('lock/stocks/inward', $this->uri->segment(4));
	...
	//Listing
	function returns(){
		$actions = checkUserPermission('lock/stocks/returns', $this->uri->segment(4));
