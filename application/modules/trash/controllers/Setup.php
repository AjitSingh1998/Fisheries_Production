...
class Setup extends MY_Controller {
	...
	//Listing
	function main_group(){
		$actions = checkUserPermission('trash/setup/main_group', $this->uri->segment(4));
		...
		$crud->columns('Name', 'Type', 'bank_name', 'branch_name', 'ifsc_code', 'account_number', 'major_fee', 'minor_fee', 'sawal_fee', 'status', 'editable');	
		...
		$this->template->set('scriptsrc', array(site_url('trash/assets/js/setup.js')));
	...
	function fishing_points(){
		$actions = checkUserPermission('trash/setup/fishing_points', $this->uri->segment(4));		
		$crud = new grocery_CRUD();
		$crud->set_theme('bootstrap');
		$crud->unset_jquery();
		$crud->unset_bootstrap();
		...
		$crud->order_by('added_date', 'DESC');
		$crud->columns('Name', 'Remark','status', 'editable');
	...
	//Listing
	function fisherman(){
		$this->actions = checkUserPermission('trash/setup/fisherman', $this->uri->segment(4));
		...
		$crud->order_by('added_date', 'DESC');
		$crud->columns('Code','Name','Contact','doc_number','Village','MainGroup','MajorFee','MinorFee','SawalFee','status', 'editable');
