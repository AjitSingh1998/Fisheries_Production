...
class Dailytoll extends MY_Controller {
	...
	function __callbackCustomActionButton($primary_key, $row){ 
		return site_url('bulk_action/action/delete_dailytoll');
	}
	...
	function index(){		
		...
		$actions = checkUserPermission('trash/dailytoll/index', $this->uri->segment(4));
		...
		if(in_array('delete', $actions)){
			$crud->add_action('Delete', 'triggerBulkDelete text-danger', '', 'fa fa-times', array($this, '__callbackCustomActionButton'), 'dialogbox');
			$crud->add_bulk_action('Delete', site_url('bulk_action/action/delete_dailytoll'), ' text-danger','fa fa-times', '');
		}
