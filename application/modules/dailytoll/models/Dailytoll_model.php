...
class Dailytoll_model extends CI_Model {
	...
	function get_fisherman($q, $f_ids = '', $offset=0, $limit=20){
		$data['result1'] = '';
		$data['result2'] = '';
	...
	function insertDailytoll($post, $userID){
		...
		if($mode == 'add' && !empty($fisherman_id)){
			$where = array('toll_date'=>$toll_date, 'CompanyId'=>$fisherman_id, 'status'=>'Active');
			$result = $this->db->select('ID, DailytollId')->where($where)->get(DAILYTOLLINFO)->result_array();
			if(!empty($result)){
				$data = array('status' => "danger", "msg" => "Error: There is already added toll data on selected date and point for this fisherman on page no - ".$result[0]['DailytollId']);
				return $data;
			}
		}
		...
		if(empty($dtollid)){			
			...
			if($mode == 'edit' && !empty($fisherman_id) && !empty($dtiid)){
				...
				$this->db->insert(DAILYTOLLINFO, $data_array);
	...
	function get_last_dailytoll($userID){
		$data = array();
		$this->db->select('ID, Date, Point');
		$this->db->from(DAILYTOLL);
